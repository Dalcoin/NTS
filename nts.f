...
c      Version 2.0.0
       ...
       dimension outpt4(100), coeff4(100), css(100), css_comp(100)
...
c       read(323,*) ngval, m, n, rho_i, rho_f, rho_fin, match, ncontrol
c       read(424,*) nde, mrk4s, des, fd, dd, format_val, ntsprint
...
c---------------------------------------------------------------------------------------------------
       ...
       c2(j) = (1/den(inac+4))*(eng(inac+4)-((alf2(j)/(gam(j)-1))*
     1    (den(inac+4)**gam(j))))
                do i=1,22
...
!              call dcsakm(n73,prs66,eng66,outpt1,coeff1)
!              call dcsakm(n73,den66,prs66,outpt2,coeff2)
!              call dcsakm(n73,den66,eng66,outpt3,coeff3)
!              call dcsakm(n73,eng66,prs66,outpt4,coeff4)
...
c                   ebdb = ced*ders(2)/(star_radius * star_radius)
                 ...
                 neos1=neos-1
c                 css(i)= dsqrt(dcsder(1,dcsval(ync,n273,outpt3,coeff3)
c     1                         ,n273,outpt4,coeff4))         
c                 css_comp(i) = dsqrt(dscval()*dscval())        
...
c                 write(000,*) ntsprint
         ...
         WRITE(666,'(E12.3,6E12.4)') cen_dens(i), radius(i)*tp, smass(i)
c                 WRITE(666,500) cen_dens(i), radius(i)*tp, smass(i)
...
c                    write(000,*) nmax, h_mms
                    ...
                    write(626,*) gam(jj),gam(j),
     1              h_mms,rad(nmax),cen_num_den,cen_eng_den,cen_prs
...
c                   Mas-14 Subgroup
                 ...
                 call dcsakm(nmax1,cnd14,rad14,dehors14red,facteur14red)
              rad_14 = dcsval(cen_den_14,nmax2,dehors14red,facteur14red)

              eng_14 = dcsval(cen_den_14,n2,dehors2,facteur2) 
              
223                continue 

                   write(727,*) gam(jj), gam(j),
     1             rad_14, cen_den_14, eng_14
          ...
          if (nstep .EQ. 1) then
             do i=1,ncd
       write(100+jj,669) set1d(i),set1r(i),set1m(i),set2r(i),set2m(i),
     1 set3r(i),set3m(i),set4r(i),set4m(i),set5r(i),set5m(i),
     1 set6r(i),set6m(i),set7r(i),set7m(i)
             end do
          end if

  669 format(E10.4,2x,E10.4,2x,F6.4,2x,E10.4,2x,F6.4,2x,E10.4,2x,F6.4,
